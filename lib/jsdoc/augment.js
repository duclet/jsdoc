...
function getDocletByLongName(doclets, longName) {
    var result = null;
    doclets.some(function(doclet) {
        ...
        if (implementations) {
            ...
            for (var j = 0, jj = implementations.length; j < jj; j++) {
                if (doclet.kind === 'class') {
                    parentDoclet = getDocletByLongName(allDoclets, implementations[j]);
                    if (parentDoclet) {
                        parentDoclet.implementations = parentDoclet.implementations || [];
                        if (parentDoclet.implementations.indexOf(doclet.longname) === -1) {
                            parentDoclet.implementations.push(doclet.longname);
                            parentDoclet.implementations.sort();
                        }
                    }
                }
